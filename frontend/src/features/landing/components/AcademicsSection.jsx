  ...
  return (
    <>
      ...
      {curriculumOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-4 backdrop-blur-sm">
          <div className="flex min-h-full items-start justify-center py-4 sm:items-center">
            <div className="flex max-h-[calc(100vh-2rem)] w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.28)]">
              <div className="bg-[linear-gradient(135deg,#f8fbff_0%,#eef6f2_45%,#ffffff_100%)] px-6 py-6 sm:px-8">
                <div className="flex items-start justify-between gap-4">
                  <div className="max-w-3xl">
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--brand)]">{t('academics.curriculumEyebrow')}</p>
                    <h3 className="mt-3 text-2xl font-black tracking-tight text-[var(--brand-strong)] sm:text-[2rem]">{t('academics.curriculumTitle')}</h3>
                    <p className="mt-3 text-sm leading-7 text-[var(--text-soft)]">{t('academics.curriculumSubtitle')}</p>
                  </div>
                  <button onClick={() => setCurriculumOpen(false)} className="rounded-2xl border border-slate-200 bg-white p-2 text-slate-500 transition hover:bg-slate-50 hover:text-slate-900" aria-label={t('academics.closeCurriculumModal')}>
                    <X className="h-5 w-5" />
                  </button>
                </div>
              </div>

              <div className="grid gap-6 overflow-y-auto px-6 py-6 sm:px-8 lg:grid-cols-[1.2fr_0.8fr]">
                <div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    {CURRICULUM_KEYS.map((key) => (
                      <div key={key} className="rounded-[1.5rem] border border-slate-100 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-5 shadow-[0_14px_30px_rgba(15,23,42,0.05)]">
                        <p className="text-[11px] font-black uppercase tracking-[0.14em] text-[var(--brand)]">{t(`academics.${key}.range`)}</p>
                        <p className="mt-3 text-lg font-black text-slate-900">{t(`academics.${key}.title`)}</p>
                        <p className="mt-2 text-sm leading-6 text-slate-500">{t(`academics.${key}.desc`)}</p>
                ...
                <div className="space-y-4">
                  <div className="rounded-[1.6rem] border border-amber-100 bg-amber-50/70 p-5">
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-amber-700">{t('academics.curriculumSupportEyebrow')}</p>
