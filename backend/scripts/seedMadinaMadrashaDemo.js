// One-time demo data seeder for Madinatunnor Madrasha.
// Unlike seedGreenfieldDemo.js, this does NOT wipe the whole database — it
// only deletes any prior Madina Madrasha tenant (cascades to that tenant's
// own rows only, via the tenant_id FKs) and re-inserts one complete,
// realistic dataset for this school, so a live sales demo has something
// coherent to click through without touching Greenfield Academy's data.
// Run with: node backend/scripts/seedMadinaMadrashaDemo.js
...
// data (helpful if it needs to be re-run right before the demo). Different
// seed than Greenfield's so the two schools don't end up with identical
// "random" picks (same class teacher assignments, same transport list, etc).
// ---------------------------------------------------------------------------
...
const TENANT_SLUG = "madina-madrasha";

// Only removes this tenant's own rows (every tenant-scoped table cascades
// from tenants.id ON DELETE CASCADE) — Greenfield Academy's tenant and data
// are untouched, unlike seedGreenfieldDemo.js's whole-database wipe.
async function wipeMadinaTenant() {
...
// ---------------------------------------------------------------------------
const CLASSES = [
  { name: "Class Six", section: "A", tuition: 1000, ageBase: 11 },
  { name: "Class Seven", section: "A", tuition: 1100, ageBase: 12 },
  { name: "Class Eight", section: "A", tuition: 1200, ageBase: 13 },
  { name: "Class Nine", section: "A", tuition: 1300, ageBase: 14 },
  { name: "Class Nine", section: "B", tuition: 1300, ageBase: 14 },
  { name: "Class Ten", section: "A", tuition: 1400, ageBase: 15 },
];

// Religious Studies is swapped for "Quran & Tajweed" — the rest mirror a
// standard Aliya-stream madrasah curriculum (general + Islamic subjects
// taught together), matching the "Quran (Tajweed), Arabic, Hadith, Fiqh,
// General Math" exam list already shown in the marketing dashboard mockups.
const SUBJECTS = [
  ...
  { code: "QUR101", name: "Quran & Tajweed", department: "Islamic Studies" },
  { code: "PHE101", name: "Physical Education", department: "Co-curricular" },
];
const CORE_SUBJECTS = ["Bangla", "English", "Mathematics", "Quran & Tajweed", "General Science"];

const TEACHERS = [
  { name: "Mawlana Nurul Amin", subject: "Quran & Tajweed", designation: "Head Teacher", department: "Islamic Studies", qualification: "Kamil, Ifta", dob: "1975-05-10", gender: "Male", salary: 42000, classTeacherOf: "Class Ten-A" },
  { name: "Sumaiya Akhter", subject: "Mathematics", designation: "Senior Teacher", department: "Mathematics", qualification: "M.Sc, B.Ed", dob: "1986-08-14", gender: "Female", salary: 36000, classTeacherOf: "Class Nine-A" },
  { name: "Ashraful Alam", subject: "English", designation: "Assistant Teacher", department: "Languages", qualification: "M.A, B.Ed", dob: "1989-02-22", gender: "Male", salary: 31000, classTeacherOf: "Class Eight-A" },
  { name: "Tania Ferdous", subject: "Information & Communication Technology", designation: "ICT Teacher", department: "ICT", qualification: "B.Sc in CSE", dob: "1992-06-30", gender: "Female", salary: 32000, classTeacherOf: "Class Nine-B" },
  { name: "Habibur Rahman", subject: "Bangla", designation: "Assistant Teacher", department: "Languages", qualification: "M.A", dob: "1983-09-18", gender: "Male", salary: 29000, classTeacherOf: "Class Seven-A" },
  { name: "Jannatul Ferdous", subject: "Bangladesh & Global Studies", designation: "Assistant Teacher", department: "Social Science", qualification: "M.S.S, B.Ed", dob: "1990-12-05", gender: "Female", salary: 29000, classTeacherOf: "Class Six-A" },
  { name: "Hafez Sirajul Karim", subject: "General Science", designation: "Assistant Teacher", department: "Science", qualification: "Kamil, B.Ed", dob: "1981-03-11", gender: "Male", salary: 28000, classTeacherOf: null },
  { name: "Rukhsana Parvin", subject: "Physical Education", designation: "Physical Education Teacher", department: "Co-curricular", qualification: "B.P.Ed", dob: "1994-01-25", gender: "Female", salary: 26000, classTeacherOf: null },
];

const STUDENT_MALE = ["Abdullah", "Yusuf", "Ibrahim", "Ismail", "Bilal", "Hamza", "Zayd", "Anas", "Musa", "Ayaan", "Rayhan", "Sami", "Tanzim", "Mahfuz", "Rakib"];
const STUDENT_FEMALE = ["Ayesha", "Maryam", "Zainab", "Ruqayyah", "Safiya", "Amina", "Khadija", "Fatima", "Sumaiya", "Tahmina", "Nusaiba", "Rabeya", "Marzia", "Lamia", "Nawshin"];
const SURNAMES = ["Islam", "Rahman", "Hossain", "Ahmed", "Khan", "Chowdhury", "Sarker", "Uddin", "Karim", "Hasan", "Alam", "Miah", "Talukder", "Bhuiyan", "Molla"];
const ADULT_MALE = ["Nazrul", "Shahjahan", "Manzur", "Faruk", "Iqbal", "Mostafa", "Wahidul", "Latiful", "Zakir", "Monirul", "Shamsul", "Ainul", "Khorshed", "Mujibur", "Ashraf"];
const ADULT_FEMALE = ["Rowshan", "Mahmuda", "Farida", "Rabeya", "Suraiya", "Tahera", "Rokshana", "Kohinoor", "Selina", "Nurjahan", "Hasina", "Morsheda", "Anjuman", "Firoza", "Shahana"];
const AREAS = ["Mirpur", "Pallabi", "Kazipara", "Shewrapara", "Kafrul", "Cantonment", "Mohammadpur", "Dhanmondi", "Gabtoli", "Rupnagar", "Vashantek", "Kallyanpur", "Agargaon", "Taltola", "Darus Salam"];
const BLOOD_GROUPS = ["A+", `A${MINUS}`, "B+", `B${MINUS}`, "O+", `O${MINUS}`, "AB+", `AB${MINUS}`];
const PREV_SCHOOLS = ["Al-Furqan Hifz Madrasha", "Bismillah Nurani Madrasha", "Green Crescent School", "Little Scholars Academy", "Daarul Uloom Maktab"];
...
async function main() {
  ...
  console.log("Connected. Removing any prior Madina Madrasha tenant (Greenfield Academy is untouched)...");
  await wipeMadinaTenant();
  console.log("Seeding Madinatunnor Madrasha demo data...");

  const now = new Date();
  const TODAY = isoDate(now.getFullYear(), now.getMonth() + 1, now.getDate());

  // --- Password hashes (shared per role tier, computed once) --------------
  const [adminHash, accountantHash, teacherHash, studentHash, guardianHash] = await Promise.all([
    hashPassword("Madina@2026"),
  ...
  // --- System developer (platform login, tenant_id NULL) — shared across
  // every tenant, so only create it if seedGreenfieldDemo.js (or a prior
  // run) hasn't already. Never delete/duplicate it here. -------------------
  const devEmail = process.env.DEFAULT_SYSTEM_DEVELOPER_EMAIL || "developer@school.local";
  const existingDev = await client.query(`SELECT id FROM users WHERE email = $1 AND tenant_id IS NULL`, [devEmail]);
  if (existingDev.rowCount === 0) {
    const devHash = await hashPassword(process.env.DEFAULT_SYSTEM_DEVELOPER_PASSWORD || "Developer@12345");
    await insertMany("users", ["id", "tenant_id", "name", "email", "password_hash", "role", "status"], [
      [createId("user"), null, process.env.DEFAULT_SYSTEM_DEVELOPER_NAME || "System Developer", devEmail, devHash, "system_developer", "active"],
    ]);
  }
  ...
  await insertMany("tenants", ["id", "name", "slug", "email", "plan", "status", "institution_type", "address", "phone"], [
    [tenantId, "Madinatunnor Madrasha", TENANT_SLUG, "info@madinatunnormadrasha.edu.bd", "pro", "active", "MADRASA", "45 Mirpur Road, Mirpur-2, Dhaka 1216, Bangladesh", "+880 1911-000000"],
  ]);
  ...
  await insertMany("subjects", ["id", "tenant_id", "code", "name", "department", "description", "is_active"],
    subjectRows.map((s) => [s.id, tenantId, s.code, s.name, s.department, `${s.name} curriculum for Madinatunnor Madrasha.`, true]));
  const subjectByName = new Map(subjectRows.map((s) => [s.name, s]));

  // --- Teachers (users + profiles) -------------------------------------------
  const teacherRecords = TEACHERS.map((t) => {
    const [first, ...rest] = t.name.replace(/^(Mawlana|Hafez|Mufti|Dr\.)\s+/, "").split(" ");
    const emailLocal = `${first.toLowerCase()}.${rest[rest.length - 1].toLowerCase()}`;
    return {
      ...t,
      userId: createId("user"),
      profileId: createId("teacher"),
      email: `${emailLocal}@madinatunnormadrasha.edu.bd`,
      employeeId: `MM-EMP-${pad(TEACHERS.indexOf(t) + 1, 3)}`,
  ...
  await insertMany("users", ["id", "tenant_id", "name", "email", "password_hash", "role", "status"], [
    [adminUserId, tenantId, "Maulana Serajul Islam", "admin@madinatunnormadrasha.edu.bd", adminHash, "admin", "active"],
    [accountantUserId, tenantId, "Md. Faridul Islam", "accounts@madinatunnormadrasha.edu.bd", accountantHash, "accountant", "active"],
  ]);
  ...
  for (const c of classRecords) {
    for (let roll = 1; roll <= 5; roll++) {
      ...
      students.push({
        ...
        email: `s${pad(globalIndex, 3)}.${firstName.toLowerCase()}@madinatunnormadrasha.edu.bd`,
        studentId: `MM-2026-${pad(globalIndex, 3)}`,
      });
    }
  }
  await insertMany("users", ["id", "tenant_id", "name", "email", "password_hash", "role", "status"],
  ...
  const sampleStudents = [students[4], students[11]];
  for (const s of sampleStudents) {
    ...
    correctionRows.push([
      ...
      "Please review the class register.", s === sampleStudents[0] ? "approved" : "pending",
  ...
  const movedStudent = students[16];
  ...
  const notices = [
    ...
    { type: "notice", audience: "public", title: "Parent-Teacher Meeting — Saturday, 18 July 2026", body: "A Parent-Teacher meeting will be held on Saturday, 18 July 2026 at 10:00 AM in the madrasha hall. All guardians are requested to attend.", publishedAt: "2026-07-08" },
    { type: "notice", audience: "public", title: "Annual Examination Routine — Classes VI to X", body: "The Annual Examination for Classes VI to X will be held from 16 November 2026. The detailed subject-wise routine, including Quran and Hadith papers, will be shared soon.", publishedAt: "2026-07-10" },
    ...
    { type: "news", audience: "public", title: "Madinatunnor Students Win Gold at District Science Fair 2026", body: "Students from Class Nine represented Madinatunnor Madrasha at the District Science Fair 2026 and won the Gold medal in the Innovation category.", publishedAt: "2026-06-20" },
    { type: "news", audience: "public", title: "New Computer Lab Inaugurated with 40 Modern Workstations", body: "Madinatunnor Madrasha inaugurated a new Computer Lab equipped with 40 modern workstations to strengthen ICT education alongside the Islamic studies curriculum.", publishedAt: "2026-05-15" },
  ];
  ...
  // --- Gallery: copy this site's own campus photos so the URLs actually resolve
  const campusSrcDir = path.join(projectRoot, "frontend", "src", "assets", "landing", "sites", "madina-madrasha", "campus");
  ...
  const applications = [
    { name: "Habiba Sultana", cls: "Class Six", status: "submitted", guardian: "Md. Kamal Uddin", phone: "01711009911", email: "kamal.uddin@gmail.com", prev: pick(PREV_SCHOOLS), notes: "" },
    { name: "Sajid Hasan", cls: "Class Seven", status: "under_review", guardian: "Rina Begum", phone: "01822110033", email: "rina.begum@gmail.com", prev: pick(PREV_SCHOOLS), notes: "Awaiting transfer certificate." },
    { name: "Tasnim Jahan", cls: "Class Six", status: "test_scheduled", guardian: "Md. Anwar Hossain", phone: "01933221144", email: "anwar.hossain@gmail.com", prev: pick(PREV_SCHOOLS), notes: "", testDate: "2026-07-20" },
    { name: "Rakibul Amin", cls: "Class Eight", status: "accepted", guardian: "Mst. Fahima Khatun", phone: "01644332255", email: "fahima.khatun@gmail.com", prev: pick(PREV_SCHOOLS), notes: "Seat confirmed for session 2026." },
    { name: "Wasim Talukder", cls: "Class Six", status: "rejected", guardian: "Md. Selim Mia", phone: "01755443366", email: "selim.mia@gmail.com", prev: pick(PREV_SCHOOLS), notes: "Seats full for the applied class." },
  ];
  ...
  const feeCategories = [
    { key: "tuition", name: "Tuition Fee", cycle: "monthly", lateFee: 100, defaultAmount: 1200 },
    { key: "transport", name: "Transport Fee", cycle: "monthly", lateFee: 50, defaultAmount: 700 },
    { key: "session", name: "Session Fee", cycle: "annual", lateFee: 0, defaultAmount: 2500 },
    { key: "exam", name: "Exam Fee", cycle: "term", lateFee: 0, defaultAmount: 400 },
    { key: "library", name: "Library Fee", cycle: "annual", lateFee: 0, defaultAmount: 250 },
  ];
  ...
  await insertMany("fee_categories", ["id", "tenant_id", "name", "description", "default_amount", "billing_cycle", "late_fee_amount", "is_active"],
    catRecords.map((c) => [c.id, tenantId, c.name, `${c.name} for Madinatunnor Madrasha.`, c.defaultAmount, c.cycle, c.lateFee, true]));
  ...
  structureRows.push([createId("fee_str"), tenantId, null, catByKey.get("session").id, 2500, true]);
  structureRows.push([createId("fee_str"), tenantId, null, catByKey.get("exam").id, 400, true]);
  structureRows.push([createId("fee_str"), tenantId, null, catByKey.get("library").id, 250, true]);
  ...
  const assignmentRows = transportStudents.map((s) => [
    createId("fee_asg"), tenantId, s.userId, catByKey.get("transport").id, 700, 0, 0, 0, 0, "2026-01", "", "active", "Opted into madrasha transport.",
  ]);
  const scholarshipStudents = [students[1], students[21]];
  for (const s of scholarshipStudents) {
    assignmentRows.push([
      createId("fee_asg"), tenantId, s.userId, catByKey.get("tuition").id, s.classRec.tuition, 0, 0, 400, 0, "2026-01", "", "active", "Hifz merit scholarship — 400 BDT monthly tuition waiver.",
    ]);
  }
  ...
  const scholarshipMap = new Map(scholarshipStudents.map((s) => [s.userId, 400]));

  // --- Invoices + items + payments --------------------------------------------
  const RECEIPT_STAMP = "20260710";
  let receiptCounter = 200000;
  ...
  for (const plan of monthPlans) {
    for (const s of students) {
      ...
      if (transportSet.has(s.userId)) items.push({ categoryId: catByKey.get("transport").id, name: "Transport Fee", amount: 700, scholarship: 0 });
      const roll = rand();
  ...
  for (const s of students) {
    const items = [
      { categoryId: catByKey.get("session").id, name: "Session Fee", amount: 2500, scholarship: 0 },
      { categoryId: catByKey.get("library").id, name: "Library Fee", amount: 250, scholarship: 0 },
    ];
    const roll = rand();
  ...
  // --- Non-teaching staff ------------------------------------------------------
  const STAFF = [
    { name: "Md. Delwar Hossain", designation: "Office Assistant", department: "Administration", qualification: "SSC", salary: 14000, staffType: "admin" },
    { name: "Nasrin Sultana", designation: "Librarian", department: "Library", qualification: "B.A in Library Science", salary: 17000, staffType: "support" },
    { name: "Md. Aynal Haque", designation: "Security Guard", department: "Security", qualification: "SSC", salary: 12500, staffType: "support" },
    { name: "Rahima Begum", designation: "Cleaner", department: "Maintenance", qualification: "Primary", salary: 9500, staffType: "non_teaching" },
    { name: "Md. Shafiqul Alam", designation: "Lab Assistant", department: "Science", qualification: "HSC", salary: 13500, staffType: "support" },
  ];
  const staffRecords = STAFF.map((s, i) => ({
    ...s, id: createId("staff"), employeeId: `MM-STF-${pad(i + 1, 3)}`,
  ...
  // --- Expenses & donations ----------------------------------------------------
  const expenseRows = [
    { category: "Utilities", payee: "DESCO", amount: 11500, date: "2026-05-05", method: "bank", notes: "Electricity bill - May 2026." },
    { category: "Utilities", payee: "DESCO", amount: 12200, date: "2026-06-05", method: "bank", notes: "Electricity bill - June 2026." },
    { category: "Utilities", payee: "WASA", amount: 3100, date: "2026-06-06", method: "cash", notes: "Water bill - June 2026." },
    { category: "Utilities", payee: "Link3 Technologies", amount: 4200, date: "2026-06-01", method: "bank", notes: "Internet bill - June 2026." },
    { category: "Supplies", payee: "Rahman Stationery House", amount: 7600, date: "2026-06-12", method: "cash", notes: "Office and classroom stationery, including Quran & Hadith copies." },
    { category: "Maintenance", payee: "City Builders", amount: 20000, date: "2026-04-18", method: "bank", notes: "Classroom ceiling and wiring repair." },
    { category: "Sports", payee: "Champion Sports House", amount: 8600, date: "2026-03-02", method: "cash", notes: "Sports equipment for Annual Sports Day." },
    { category: "Lab Supplies", payee: "Scientific Traders", amount: 6100, date: "2026-05-20", method: "nagad", notes: "Science lab chemicals and glassware." },
    { category: "Printing", payee: "Modern Printing Press", amount: 4900, date: "2026-06-25", method: "cash", notes: "Half-Yearly exam question papers and mark sheets." },
  ];
  ...
  const donationRows = [
    { donor: "Madinatunnor Alumni Association", amount: 20000, date: "2026-01-25", method: "bank", notes: "Annual alumni contribution toward library development." },
    { donor: "Md. Anisur Rahman (Guardian)", amount: 5000, date: "2026-03-10", method: "cash", notes: "Contribution toward Science Fair expenses." },
    { donor: "Noor Welfare Foundation", amount: 12000, date: "2026-06-02", method: "bank", notes: "Support for Hifz-track student scholarships." },
  ];
  ...
  const commThreads = [
    ...
    {
      topic: "Homework clarification - Mathematics", student: students[12], teacherName: "Sumaiya Akhter", guardianSide: true,
      messages: [
    ...
    },
    {
      ...
      messages: [
        { fromGuardian: true, body: "I would like to request a short meeting regarding my daughter's progress in Quran recitation before the PTM date if possible." },
    ...
    },
    {
      ...
      messages: [
        { fromGuardian: true, body: "Does the madrasha transport cover the Kazipara route in the morning? What time does it arrive?" },
        { fromGuardian: false, body: "Yes, the Kazipara route pickup is around 7:10 AM. I'll share the full route sheet with you." },
      ],
    },
  ];
  ...
  await insertMany("contact_messages", ["id", "name", "phone", "message", "status"], [
    [createId("contact"), "Md. Rafiqul Islam", "01711223344", "Assalamu Alaikum, I want to know the admission process and Hifz program details for Class Six for my son. Please advise the required documents and fees.", "NEW"],
    [createId("contact"), "Sultana Yasmin", "01822334455", "Hello, does Madinatunnor Madrasha provide transport for the Kazipara area? Please let me know the fare.", "RESOLVED"],
  ]);

  // --- Backfill finance_transactions ledger from what we just inserted -------
  // Idempotent (ON CONFLICT DO NOTHING per source row) and scans every tenant,
  // so this only adds the rows Madina just needs — Greenfield's are untouched.
  console.log("Backfilling finance ledger...");
  await createSchema(client);

  console.log("Done. Seed summary:");
  console.log(`  Tenant: Madinatunnor Madrasha (${tenantId})`);
  console.log(`  Admin login:      admin@madinatunnormadrasha.edu.bd / Madina@2026`);
  console.log(`  Accountant login: accounts@madinatunnormadrasha.edu.bd / Accounts@2026`);
  console.log(`  Any teacher login (see teacher_profiles): <first>.<last>@madinatunnormadrasha.edu.bd / Teacher@2026`);
  console.log(`  Any student login: s0XX.<first>@madinatunnormadrasha.edu.bd / Student@2026`);
  console.log(`  Any guardian login: g0XX.<first>@gmail.com / Guardian@2026`);
  console.log(`  System developer:  ${devEmail} / (unchanged — set on first-ever seed run)`);
}
